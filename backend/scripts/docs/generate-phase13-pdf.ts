...
const FINAL_MD_PATH = path.join(DOCS_DIR, 'GETRA_Final_System_Documentation.md');
const FINAL_PDF_PATH = path.join(DOCS_DIR, 'documentation phase intergation.pdf');
...
(async () => {
  try {
    ...
    const fullHtml = `
      ...
      <head>
        <meta charset="utf-8">
        <title>GETRA Phase 13 Documentation</title>
        <style>
          body {
