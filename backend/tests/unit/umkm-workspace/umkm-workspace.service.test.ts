...
function database(tables: Record<string, any[]>, failedTable?: string) {
  ...
  const client = {
    ...
    from: vi.fn((table: string) => {
      ...
      const query: any = {
        ...
        order: vi.fn((key: string) => {
          if (key !== "id") rows.sort((a, b) => String(b[key]).localeCompare(String(a[key])));
          return query;
        }),
  ...
  return { service: new UmkmWorkspaceService(client as any), ranges, client };
}

const merchant = (id: string, owner = "owner") => ({
  id,
  owner_id: owner,
  name: `Usaha ${id}`,
  address: null,
...
const submission = (id: string, status: string, updated = "2026-08-01") => ({
  id,
  submitted_by: "owner",
  name: `Pendaftaran ${id}`,
  category: "Kuliner",
...
describe("UmkmWorkspaceService", () => {
  ...
  it("returns NO_MERCHANT inputs when there are no owned businesses or workflows", async () => {
    ...
    expect(await service.getWorkspaceSummary("owner")).toEqual({
  ...
  it("keeps active campaign counts scoped to each owned merchant", async () => {
    const { service } = database({
      merchants: [merchant("one"), merchant("two"), merchant("private", "someone-else")],
      ad_campaigns: [
        { id: "a", merchant_id: "one", status: "ACTIVE" },
        { id: "b", merchant_id: "one", status: "PAUSED" },
        { id: "c", merchant_id: "two", status: "DRAFT" },
        { id: "d", merchant_id: "private", status: "ACTIVE" },
      ],
    });
  ...
  it.each(["PENDING", "APPROVED"])("a %s claim without owner_id never grants workspace access", async (status) => {
    const { service } = database({
      merchants: [merchant("external", "someone-else")],
      merchant_claims: [claim(status)],
    });
    const summary = await service.getWorkspaceSummary("owner");
    expect(summary.owned_merchants).toEqual([]);
  ...
  it("reads the category_label stored by registration approval before description", async () => {
    const approved = {
      ...merchant("approved"),
      metadata: { category_label: "Makanan & Minuman" },
      description: "Deskripsi panjang usaha",
    };
  ...
  it("paginates owned businesses and campaigns instead of reporting a truncated total", async () => {
    const { service, ranges } = database({
      merchants: Array.from({ length: 101 }, (_, index) => merchant(`owner-${index}`)),
      ad_campaigns: Array.from({ length: 102 }, (_, index) => ({
        id: `campaign-${index}`,
        merchant_id: "owner-0",
        status: "ACTIVE",
      })),
  ...
  it("retains old pending claims and submissions plus drafts beyond recent history and page boundaries", async () => {
    ...
    const { service, ranges } = database({
      merchants: [merchant("external", "other")],
      merchant_submissions: [...history, submission("pending", "PENDING_REVIEW"), submission("draft", "DRAFT")],
      merchant_claims: [
        ...Array.from({ length: 15 }, (_, index) => ({ ...claim("REJECTED"), id: `closed-${index}`, created_at: "2026-09-01" })),
        claim("PENDING"),
      ],
    });
    const summary = await service.getWorkspaceSummary("owner");
    ...
    expect(summary.recent_submissions).toEqual(
      expect.arrayContaining([expect.objectContaining({ id: "pending" }), expect.objectContaining({ id: "draft" })]),
    );
  ...
  it.each(["merchant_submissions", "merchant_claims", "ad_campaigns"])(
    "does not turn a failed %s query into empty/zero state",
    async (failedTable) => {
  ...
  it("refreshes actual ownership after approval while preserving other pending workflows", async () => {
    const tables = {
      merchants: [merchant("external", "someone-else")],
      merchant_claims: [claim("PENDING")],
      merchant_submissions: [submission("another", "PENDING_REVIEW")],
    };
  ...
  it("archives a merchant through the owner-scoped database function", async () => {
    ...
    const client = { rpc: mockRpc, from: vi.fn() };
    const service = new UmkmWorkspaceService(client as any);
    ...
    expect(mockRpc).toHaveBeenCalledWith("archive_owned_merchant", {
