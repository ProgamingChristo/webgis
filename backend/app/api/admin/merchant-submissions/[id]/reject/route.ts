...
export async function POST(
  ...
  return withApiLogger(req, reqId, async () => {
    ...
    if (!parsed.success) {
      const errMsg = parsed.error.issues[0]?.message || "Alasan penolakan wajib diisi.";
    ...
    const result = await service.adminReject(id, adminId, parsed.data.note);

    return createSuccessResponse(reqId, result);
  });
}

export const OPTIONS = createOptionsHandler("/api/admin/merchant-submissions/[id]/reject");
