...
export class BusinessSpaceInsightService {
  async explain(comparison: BusinessSpaceComparison, question = "Jelaskan trade-off lokasi kandidat."): Promise<BusinessSpaceInsight> {
...
Explain trade-offs between candidates and mention evidence quality when metrics are unavailable.`,
    ...
    return response(
      comparison,
      answer && !FORBIDDEN_CLAIM.test(answer) && numbersIn(answer).every((item) => allowedNumbers(comparison).has(item))
        ? "AI"
        : "DETERMINISTIC_FALLBACK",
      answer && !FORBIDDEN_CLAIM.test(answer) ? answer : fallback,
    );
  }
}
...
function deterministicInsight(comparison: BusinessSpaceComparison) {
  return `${comparison.trade_off_summary} Semua kandidat memakai kategori ${comparison.category_slug}, window ${comparison.days} hari, dan model ${comparison.model_version}.`;
}
...
function allowedNumbers(comparison: BusinessSpaceComparison) {
  const values = comparison.candidates.flatMap((item) => [
    ...
    item.market_context.demand_score,
    item.market_context.supply_score,
    item.market_context.retail_gap,
