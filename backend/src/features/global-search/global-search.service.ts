...
export class GlobalSearchService {
  ...
  async search(query: GlobalSearchQuery): Promise<GlobalSearchResult> {
    ...
    const intent = resolveGlobalSearchIntent(
      query,
      availableRegions,
    );

    const spatialSearch = Boolean(
    ...
    if (spatialSearch) {
      const reference = await resolveSearchReference(
        this.supabase,
        query.reference_text,
        intent.origin,
      );
      ...
      if (
        ...
        throw new ApplicationError(
          "VALIDATION_ERROR",
          "Aktifkan lokasi atau pilih acuan pencarian.",
        );
      }
    ...
    const globalScope =
      intent.scope.type === "GLOBAL";

    const page =
      await new CanonicalMerchantReadService(
        this.supabase,
      ).list({
        west: globalScope
          ? undefined
          : intent.scope.bounds.west,

        south: globalScope
          ? undefined
          : intent.scope.bounds.south,

        east: globalScope
          ? undefined
          : intent.scope.bounds.east,

        north: globalScope
          ? undefined
          : intent.scope.bounds.north,

        limit: hasHardConstraints
          ? candidateLimit
          : query.limit,

        offset: hasHardConstraints
          ? 0
          : query.offset,
        ...
        origin: intent.reference
          ? {
              longitude:
                intent.reference.longitude,
              latitude:
                intent.reference.latitude,
            }
          : undefined,
      });
    ...
    const metadata: CommuterSearchMetadata = {
      ...
      regions: availableRegions.filter(
        (region) =>
          intent.scope.region_ids.includes(
            region.id,
          ),
      ),
    };
      ...
      merchants = merchants.map(
        (merchant) => ({
          ...
          referenceDistance: {
            ...
            kind: "STRAIGHT_LINE" as const,
          },
        }),
      );

      if (query.radius_meters) {
        metadata.hard_constraints_applied.push(
          "REFERENCE_RADIUS_METERS",
        );

        merchants = merchants.filter(
          (merchant) =>
            merchant.referenceDistance!
              .meters <=
            query.radius_meters!,
        );
      }
    ...
    if (
      ...
      metadata.hard_constraints_applied.push(
        "OPEN_NOW_ASIA_JAKARTA",
      );

      merchants = merchants.filter(
        (merchant) => {
          if (
            merchant.openingStatus ===
            "UNKNOWN"
          ) {
            metadata.excluded
              .unknown_hours += 1;
    ...
    if (
      ...
      metadata.hard_constraints_applied.push(
        "MAX_NETWORK_WALKING_MINUTES",
      );

      const walking =
        await new CommuterNetworkRepository(
          this.supabase,
        ).walkingCosts(
          intent.origin,
      ...
      merchants = merchants.filter(
        (merchant) => {
          ...
          merchant.networkRouteStatus =
            "ROUTABLE";

          merchant.networkDistanceMeters =
    ...
    /**
     * Recommendation ranking operates only
     * on the filtered canonical merchant set.
     *
     * Sponsorship MUST NOT affect this
    ...
    if (query.recommendation) {
      ...
      merchants.sort(
        (a, b) =>
          (
            a.observedPriceAmount ??
            Infinity
          ) -
            (
              b.observedPriceAmount ??
              Infinity
            ) ||
    ...
    metadata.constrained_count =
      merchants.length;

    const resultMerchants =
      hasHardConstraints
        ? merchants.slice(
            query.offset,
            query.offset +
  ...
  private async listRegions(): Promise<
    SearchRegion[]
  > {
    const {
      data,
...
export function resolveGlobalSearchIntent(
  ...
  const queryLocation =
    extractRegionFromQuery(
      parsedCommuter.keyword_text,
    );

  /**
   * Shared query-resolution pipeline.
   *
   * Handles:
   * - normalization
   * - known aliases
  ...
  if (
    regionIds.some(
      (id) =>
        !GLOBAL_SEARCH_REGION_IDS.includes(
          id as never,
        ),
    )
  ) {
  ...
  if (
    ...
    if (
      ...
      if (
        ...
        const latDelta = (effectiveRadiusMeters / 111_320) * 1.1;
        const cosLat = Math.cos((query.origin_latitude * Math.PI) / 180);
        const lngDelta = (effectiveRadiusMeters / (111_320 * (cosLat || 1))) * 1.1;
        bounds = {
  ...
  return {
     ...
     * IMPORTANT:
     *
     * Always search using the canonicalized
     * query produced by resolveSearchQuery().
     *
     * Example:
     *
     * raw:
    ...
    confidence:
      parsedCommuter.confidence,

    /**
     * Keep the resolution evidence so
     * frontend/API can explain typo correction.
     *
     * Example:
     *
     * normalized = "basko"
    ...
    query_resolution: {
      normalized:
        queryResolution.normalized,

      canonical:
        queryResolution.canonical,

      correction:
        queryResolution.correction,

      confidence:
        queryResolution.confidence,
    },
  };
}
