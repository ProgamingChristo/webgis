...
create or replace function public.search_canonical_merchants_v2(
...
as $$
  with input as (
    select
      ...
      (p_origin_lng is not null and p_origin_lat is not null and p_radius_meters is not null and p_radius_meters > 0) as has_radius_filter,
      (p_origin_lng is not null and p_origin_lat is not null) as has_origin
  ),
  candidates as (
    select distinct
      merchant.id,
    ...
    where merchant.publish_status = 'PUBLISHED'::public.publish_status
      and (
        -- 1. Radius nearby filter takes precedence if specified
        (
          input.has_radius_filter
        ...
        -- 2. Explicit administrative regions
        or (
          not input.has_radius_filter
          and cardinality(input.requested_regions) > 0
          and exists (
            select 1
        ...
        -- 4. Global search by keyword without bbox/region
        or (
          not input.has_radius_filter
          and cardinality(input.requested_regions) = 0
  ...
  ranked as (
    select
      ...
      case
        when (select has_origin from input) then
          extensions.st_distance(
            candidate.location::extensions.geography,
            extensions.st_setsrid(extensions.st_makepoint(p_origin_lng, p_origin_lat), 4326)::extensions.geography
          )
        else null
  ...
  from ranked
  order by
    case when (select has_radius_filter from input) then ranked.distance_meters end asc nulls last,
...
revoke all on function public.search_canonical_merchants_v2(
  double precision,
  double precision,
  double precision,
  double precision,
...
grant execute on function public.search_canonical_merchants_v2(
  double precision,
  double precision,
  double precision,
  double precision,
