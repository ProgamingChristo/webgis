...
as $$
  ...
  candidates as (
    select distinct
      ...
      case
        ...
        when lower(merchant.name) = input.keyword then 500
        when starts_with(lower(merchant.name), input.keyword) then 400
        when strpos(lower(merchant.name), input.keyword) > 0 then 320
        when strpos(lower(coalesce(merchant.metadata->>'category', '')), input.keyword) > 0 then 260
        when strpos(lower(coalesce(menu_observation.search_text, '')), input.keyword) > 0 then 240
        else greatest(
          round(extensions.similarity(lower(merchant.name), input.keyword) * 200)::integer,
          round(extensions.word_similarity(input.keyword, lower(merchant.name)) * 180)::integer,
          round(extensions.word_similarity(input.keyword, lower(coalesce(menu_observation.search_text, ''))) * 160)::integer
        )
    ...
    where merchant.publish_status = 'PUBLISHED'::public.publish_status
      and (
        (cardinality(input.requested_regions) > 0 and exists (
          select 1 from public.administrative_regions requested_region
        ...
        or (cardinality(input.requested_regions) = 0 and input.bbox_parts = 4 and extensions.st_intersects(
          merchant.location,
          extensions.st_makeenvelope(p_west, p_south, p_east, p_north, 4326)
        ))
        or (cardinality(input.requested_regions) = 0 and input.bbox_parts = 0 and input.keyword is not null)
      )
      and (
        ...
        or extensions.similarity(lower(merchant.name), input.keyword) >= 0.35
        or extensions.word_similarity(input.keyword, lower(merchant.name)) >= 0.55
        or extensions.word_similarity(input.keyword, lower(coalesce(menu_observation.search_text, ''))) >= 0.65
      )
      and (
  ...
  ranked as (
    select candidate.id, candidate.name, candidate.relevance_score,
  ...
  select ranked.id, count(*) over (), ranked.relevance_score, ranked.region_ids, ranked.region_names
  from ranked
