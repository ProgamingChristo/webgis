...
export interface ContextualLayerVisibility {
  merchant: boolean;
  property: boolean;
  transaction: boolean;
  activities: boolean;
...
export type ContextualLayerKey = keyof ContextualLayerVisibility;

export interface ContextualSourceState {
  collection: ContextualObservationCollection;
  error: string | null;
  loading: boolean;
  totalAvailable: number;
}

export interface ContextualLayerData {
  PROPERTI_GO: ContextualSourceState;
  STRUK_GO: ContextualSourceState;
  ACTIVITIES: ContextualSourceState;
}
