...
export function useActiveJourney(destination: Coordinate | null, mode: RoutingMode, preference: RoutePreference = "FASTEST") {
  ...
  useEffect(() => {
    controller.configure({ destination: lat !== undefined && lon !== undefined ? { latitude: lat, longitude: lon } : null, mode, preference });
  }, [controller, lat, lon, mode, preference]);
  useEffect(() => {
  ...
  const key = JSON.stringify([lat, lon, mode, preference]);
